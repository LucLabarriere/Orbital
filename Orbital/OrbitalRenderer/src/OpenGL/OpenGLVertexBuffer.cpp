...
namespace Orbital
{
	auto VertexBuffer::Create(size_t bufferSize, size_t vertexSize, const void* data)
		-> VertexBuffer*
	{
	...
	OpenGLVertexBuffer::OpenGLVertexBuffer(
		size_t bufferSize, size_t vertexSize, const void* data
	)
	...
	void OpenGLVertexBuffer::addVertexAttribute(
		size_t componentCount, size_t attributeSize
	)
	{
		glad_glBindBuffer(GL_ARRAY_BUFFER, mRendererID);
		glad_glEnableVertexAttribArray(mVertexAttributeCount);
		glad_glVertexAttribPointer(
			mVertexAttributeCount, componentCount, GL_FLOAT, GL_FALSE, mVertexSize,
			(GLvoid*)mStride
		);
