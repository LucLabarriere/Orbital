...
namespace FPSDemo
{
	...
	void PlayerController::onUpdate(const Time& dt)
	{
		...
		if (Inputs::IsKeyDown(OE_KEY_S))
		{
			...
			tempTransform.position.z -= mSpeed * dt.seconds();
		}
		...
		this->camera.get<CameraComponent>()->setMainVector(Maths::Vec3(0.0f, 0.0f, 1.0f));
		camera.get<TransformComponent>()->position = tempTransform.position;
	}
